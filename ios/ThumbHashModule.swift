...
@objc(ThumbHashModule)
class ThumbHashModule: NSObject {
  @objc
  func decodeThumbHash(_ base64Hash: String, resolver: @escaping RCTPromiseResolveBlock, rejecter: @escaping RCTPromiseRejectBlock) {
    guard let data = Data(base64Encoded: base64Hash) else {
      rejecter("E_INVALID_HASH", "Invalid base64 ThumbHash string", nil)
      return
    }
    ...
    guard let pngData = image.pngData() else {
      rejecter("E_IMAGE_FAIL", "Failed to convert image to PNG", nil)
      return
    }
  ...
  @objc
  func encodeThumbHash(_ base64Image: String, resolver: @escaping RCTPromiseResolveBlock, rejecter: @escaping RCTPromiseRejectBlock) {
    guard let imageData = Data(base64Encoded: base64Image),
          let image = UIImage(data: imageData) else {
      rejecter("E_INVALID_IMAGE", "Invalid base64 image string", nil)
      return
    }

    let thumbHashData = imageToThumbHash(image: image)
    let base64ThumbHash = thumbHashData.base64EncodedString()
    resolver(base64ThumbHash)
  }
